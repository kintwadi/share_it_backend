...
- Create an HTTP interceptor that:
  ...
  - supports forwarding the tenant header (`X-Tenant-ID` by default) when the backend is running in multi-tenant mode, using a tenant id that matches a configured `tenants.config.*` key such as `vicinity24_tenant_a`
