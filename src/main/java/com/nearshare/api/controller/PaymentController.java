...
import com.nearshare.api.dto.PaymentTransactionDTO;
...
public class PaymentController {
    ...
    public ResponseEntity<List<PaymentTransactionDTO>> getMyTransactions(
        ...
        List<com.nearshare.api.model.Transaction> payerTx = transactionRepository.findByPayerIdOrderByTimestampDesc(userId);
        List<com.nearshare.api.model.Transaction> payeeTx = transactionRepository.findByPayeeIdOrderByTimestampDesc(userId);

        java.util.Map<UUID, PaymentTransactionDTO> merged = new java.util.HashMap<>();

        for (com.nearshare.api.model.Transaction t : payerTx) {
            if (t == null || t.getId() == null) continue;
            merged.put(t.getId(), toPaymentTxDTO(t, userId));
        }
        for (com.nearshare.api.model.Transaction t : payeeTx) {
            if (t == null || t.getId() == null) continue;
            merged.put(t.getId(), toPaymentTxDTO(t, userId));
        }
    ...
    public ResponseEntity<Map<String, String>> getTransactionInvoiceUrl(
        ...
        com.nearshare.api.model.Transaction tx = transactionRepository.findById(transactionId).orElse(null);
        if (tx == null) {
    ...
    private PaymentTransactionDTO toPaymentTxDTO(com.nearshare.api.model.Transaction t, UUID currentUserId) {
