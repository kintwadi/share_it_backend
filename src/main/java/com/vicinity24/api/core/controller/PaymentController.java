...
public class PaymentController {
    ...
    public ResponseEntity<List<PaymentTransactionDTO>> getMyTransactions(
        ...
        List<Transaction> payerTx = transactionRepository.findByPayerIdOrderByTimestampDesc(userId);
        List<Transaction> payeeTx = transactionRepository.findByPayeeIdOrderByTimestampDesc(userId);

        java.util.Map<UUID, PaymentTransactionDTO> merged = new java.util.HashMap<>();

        for (Transaction t : payerTx) {
            if (t == null || t.getId() == null) continue;
            merged.put(t.getId(), toPaymentTxDTO(t, userId));
        }
        for (Transaction t : payeeTx) {
            if (t == null || t.getId() == null) continue;
            merged.put(t.getId(), toPaymentTxDTO(t, userId));
        }
    ...
    public ResponseEntity<Map<String, Object>> createPaymentIntent(
        ...
        if (listing.getType() == ListingType.GIVE) {
    ...
    private PaymentTransactionDTO toPaymentTxDTO(Transaction t, UUID currentUserId) {
