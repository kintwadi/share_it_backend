...
import com.vicinity24.api.util.GeohashUtil;
...
public class ListingService {
    ...
    @Transactional
    public ListingDTO borrow(UUID id, User borrower, com.vicinity24.api.dto.BorrowRequest request) {
        Listing l = listingRepository.findById(id).orElseThrow(() -> new RuntimeException("listing_not_found"));
        ...
        if (amount.compareTo(BigDecimal.ZERO) > 0 && request.getPaymentMethod() != null && !"CASH".equalsIgnoreCase(request.getPaymentMethod())) {
             ...
             com.vicinity24.api.model.Transaction t = com.vicinity24.api.model.Transaction.builder()
                 .id(UUID.randomUUID())
                 .listing(l)
                 .payer(borrower)
                 .payee(l.getOwner())
    ...
    public void dismiss(User current, UUID listingId) {
        ...
        var rec = com.vicinity24.api.model.RecommendationDismiss.builder().id(UUID.randomUUID()).user(current).listing(l).createdAt(java.time.LocalDateTime.now()).build();
